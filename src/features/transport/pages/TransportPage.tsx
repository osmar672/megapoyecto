...
export function TransportPage() {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => setBuses(transportRepository.advanceSimulation()), 8000);
    ...
    <div className={styles.page}>
      ...
      <section className={styles.grid} aria-live="polite">
        {buses.length === 0 ? <EmptyState>No hay buses registrados.</EmptyState> : buses.map((bus) => {
          const route = transportRepository.listRoutes().find((candidate) => candidate.id === bus.routeId);
...
export function BusSchedulesPage() {
  const routes = transportRepository.listRoutes();
  const [query, setQuery] = useState("");
  const filtered = routes.filter((route) => `${route.name} ${route.stops.map((stop) => stop.name).join(" ")}`.toLocaleLowerCase("es").includes(query.toLocaleLowerCase("es")));
